from net.network import SelfAttention, SAVANet, Encoder, SANet, SAVANet_test
import torch
...
class Transform_SA(nn.Module):
    def __init__(self, in_channel):
        super(Transform_SA, self).__init__()
        self.sanet4_1 = SANet(in_channel=in_channel)
        self.sanet5_1 = SANet(in_channel=in_channel)
        self.upsample5_1 = nn.Upsample(scale_factor=2, mode='nearest')
        self.merge_conv_pad = nn.ReflectionPad2d((1, 1, 1, 1))
        self.merge_conv = nn.Conv2d(in_channel, in_channel, (3, 3))

    def forward(self, content4_1, style4_1, content5_1, style5_1):
        O4_1 = self.sanet4_1(content4_1, style4_1)
        O5_1 = self.sanet5_1(content5_1, style5_1)
        return self.merge_conv(self.merge_conv_pad(
            O4_1 + self.upsample5_1(O5_1)))
...
class Transform_test(nn.Module):
    def __init__(self, in_channel = 512, self_attn = None, alpha=0.5, filter=False, transformer_sa = None):
        super(Transform_test, self).__init__()
        self.savanet4_1 = SAVANet_test(in_channel=in_channel, self_attn = self_attn, alpha=alpha, filter = filter, sanet=transformer_sa.sanet4_1)
        self.savanet5_1 = SAVANet_test(in_channel=in_channel, self_attn = self_attn, alpha=alpha, filter = filter,  sanet=transformer_sa.sanet5_1)
        self.upsample5_1 = transformer_sa.upsample5_1

        self.merge_conv_pad = transformer_sa.merge_conv_pad
        self.merge_conv = transformer_sa.merge_conv
...
class SA_test(nn.Module):
    def __init__(self, transformer = None, encoder = None, decoder = None):
        super(SA_test, self).__init__()
        self.transformer = transformer
        self.encode = Encoder() if encoder == None else Encoder(encoder)
        self.decode = decoder
    ...
    def forward(self, contents, styles):
        ...
        swapped_features = self.transformer(content_hidden_feature_4, content_hidden_feature_4, content_hidden_feature_5, content_hidden_feature_5)
        I_cc = self.decode(swapped_features)
        F_cc = self.encode(I_cc)

        swapped_features = self.transformer(style_hidden_feature_4, style_hidden_feature_4, style_hidden_feature_5, style_hidden_feature_5)
        I_ss = self.decode(swapped_features)
        F_ss = self.encode(I_ss)
